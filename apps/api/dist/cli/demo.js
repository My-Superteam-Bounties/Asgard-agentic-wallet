...
function log(icon, label, msg, color = COLORS.cyan) {
...
async function main() {
    ...
    const agentResponse = await axios_1.default.post(`${ASGARD_BASE_URL}/agents`, { name: 'TradingBot-DEMO', policyProfile: 'default' }, { headers: { Authorization: `Bearer ${ADMIN_KEY}` } });
    ...
    const balanceResponse = await axios_1.default.get(`${ASGARD_BASE_URL}/wallet/${agentId}/balance`, { headers: { Authorization: `Bearer ${apiKey}` } });
    ...
    await delay(500);
    try {
        const swapResponse = await axios_1.default.post(`${ASGARD_BASE_URL}/intent/swap`, {
            inputToken: 'USDC',
            outputToken: 'SOL',
            amount: 10,
            slippageBps: 50,
